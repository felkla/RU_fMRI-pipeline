...
for s = 1:numel(all_sub_ids) % across volunteers
    ...
    mkdir(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no));%
    for ses = 1:numel(import.prisma{ind_c})
        fi = 1;ftp_cmd = [];
        Volunteer(s).sess(ses).ID = sprintf('%s_%d',scanner,import.prisma{ind_c}{ses});
        ftp_file = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('%s_ftp.txt',Volunteer(s).sess(ses).ID)); %create SFTP batch file
        ...
        for tt = 1:numel(import.data)
            ...
            for g = ind
                if ~isfolder(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir))
                    mkdir(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses)),import.data(tt).dir); % create BIDS dir
                end
                ...
                if eval(import.data(tt).cond)
                    if isempty(import.prisma_skip{ind_c}{ses}) | ~ismember(ser,import.prisma_skip{ind_c}{ses}) % check exclusions
                        mkdir(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir),sprintf('%s_%d_%d',import.data(tt).type,ses,run)); % create temp dir for dicom files
                        ...
                        ftp_cmd{fi} = ['lcd ' fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run))]; fi = fi + 1;
        ...
        for tt = 1:numel(Volunteer(s).sess(ses).data)
            for run = 1:numel(Volunteer(s).sess(ses).data{tt})
                all_dicom =  spm_select('FPlist', fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)),'^MR.*');
                ...
                final_del_dir = strvcat(final_del_dir,fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)));
                matlabbatch{g}.spm.util.import.dicom.outdir = {fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run))};
        ...
        for tt = 1:numel(Volunteer(s).sess(ses).data)
            for run = 1:numel(Volunteer(s).sess(ses).data{tt})
                switch Volunteer(s).sess(ses).data{tt}(run).mod
                    case 'func'
                        ff = dir(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run),'*.nii'));
                        if size(unique(cat(1,ff.bytes)),1) == 2 % indicates we have 2 series eg spinal + brain, where size(spinal)<size(brain)
                            ...
                            large_f = spm_file(strvcat(ff(large).name),'path',fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)));
                            ...
                            matlabbatch{g}.spm.util.cat.name = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_acq-%s_bold.nii',Volunteer(s).no,ses,vars.task,run,'brain'));
                            matlabbatch{g}.spm.util.cat.dtype = 4;
                            matlabbatch{g}.spm.util.cat.RT = vars.sliceTiming.tr;
                            g = g + 1;
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.inputs{1}.string = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_acq-%s_bold.nii',Volunteer(s).no,ses,vars.task,run,'brain'));
                            ...
                            g = g + 1;
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.files = cellstr(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_acq-%s_bold.nii',Volunteer(s).no,ses,vars.task,run,'brain')));
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.outdir = cellstr(fullfile(path.backupDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.keep = true;
                            g = g + 1;
                            small_f = spm_file(strvcat(ff(~large).name),'path',fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)));
                            % small_f(1:import.dummies,:) = []; %remove dummies
                            matlabbatch{g}.spm.util.cat.vols = cellstr(small_f);
                            matlabbatch{g}.spm.util.cat.name = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_acq-%s_bold.nii',Volunteer(s).no,ses,vars.task,run,'spinal'));
                            matlabbatch{g}.spm.util.cat.dtype = 4;
                            matlabbatch{g}.spm.util.cat.RT = vars.sliceTiming.tr;
                            g = g + 1;
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.inputs{1}.string = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_acq-%s_bold.nii',Volunteer(s).no,ses,vars.task,run,'spinal'));
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.inputs{2}.string = dat_tim;
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.outputs = {};
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.fun = 'add_time_epi';
                            g = g + 1;
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.files = cellstr(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_acq-%s_bold.nii',Volunteer(s).no,ses,vars.task,run,'spinal')));
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.outdir = cellstr(fullfile(path.backupDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.keep = true;
                            g = g + 1;
                            
                        else % just one EPI e.g. brain
                            all_f = spm_file(strvcat(ff.name),'path',fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)));
                            ...
                            matlabbatch{g}.spm.util.cat.name = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_bold.nii',Volunteer(s).no,ses,vars.task,run));
                            matlabbatch{g}.spm.util.cat.dtype = 4;
                            matlabbatch{g}.spm.util.cat.RT = vars.sliceTiming.tr;
                            g = g + 1;
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.inputs{1}.string = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_bold.nii',Volunteer(s).no,ses,vars.task,run));
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.inputs{2}.string = dat_tim;
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.outputs = {};
                            matlabbatch{g}.cfg_basicio.run_ops.call_matlab.fun = 'add_time_epi';
                            g = g + 1;
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.files = cellstr(fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_task-%s_run-%2.2d_bold.nii',Volunteer(s).no,ses,vars.task,run)));
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.outdir = cellstr(fullfile(path.backupDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                            matlabbatch{g}.cfg_basicio.file_dir.file_ops.cfg_gzip_files.keep = true;
                            g = g + 1;
                        end
                    case  'fmap'
                        source = spm_select('FPlist',fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)),sprintf('^s%s.*\\.nii$',scanner));
                        switch import.data(tt).type
                            case {'epi_std'}
                                target = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_dir-%s_run-%2.2d_%s.nii',Volunteer(s).no,ses,'std',run,'epi'));
                                ...
                                mkdir(fullfile(path.baseDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                                spm_copy(target,fullfile(path.backupDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir),'gzip',true,'nifti',true);
                            case {'epi_inv'}
                                target = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_dir-%s_run-%2.2d_%s.nii',Volunteer(s).no,ses,'inv',run,'epi'));
                                descr = struct('PhaseEncodingDirection','j-','TotalReadoutTime',NaN,'B0FieldIdentifier','pepolar_fmap0');
                                spm_jsonwrite(spm_file(target,'ext','json'),descr, struct('indent','  ')); % create description file
                                copyfile(source,target);
                                mkdir(fullfile(path.baseDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                            ...
                            case 'magnitude'
                                ...
                                for si=1:size(source,1)
                                    target = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_run-%2.2d_%s%d.nii',Volunteer(s).no,ses,run,import.data(tt).type,si));
                                    copyfile(source(si,:),target);
                                    b = spm_jsonread(spm_file(source(si,:),'ext','json'));
                                    echo_times(si) = b.acqpar.EchoTime./1000; % echo time in s
                                    mkdir(fullfile(path.baseDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                                    spm_copy(target,fullfile(path.backupDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir),'gzip',true);
                                end
                            case 'phasediff'
                                target = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_run-%2.2d_%s.nii',Volunteer(s).no,ses,run,import.data(tt).type));
                                copyfile(source,target);
                    ...
                    case 'anat'
                        source = spm_select('FPlist',fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('%s_%d_%d',import.data(tt).type,ses,run)),sprintf('^s%s.*\\.nii$',scanner));
                        target = fullfile(path.preprocDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir,sprintf('sub-%2.3d_ses-%2.2d_%s.nii',Volunteer(s).no,ses,import.data(tt).type));
                        copyfile(source,target);
                        mkdir(fullfile(path.baseDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir));
                        spm_copy(target,fullfile(path.backupDir,sprintf('sub-%2.3d',Volunteer(s).no),sprintf('ses-%2.2d',ses),import.data(tt).dir),'gzip',true);
                    case 'blip'
