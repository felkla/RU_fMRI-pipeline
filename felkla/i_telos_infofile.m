...
% my paths
padi.main = fullfile('/home','klaassen','projects','telos');
padi.rawdata = fullfile(padi.main,'Raw');
padi.data = fullfile('/projects','crunchie','telos','data');
...
padi.trialvals = fullfile(padi.main,'Scripts','Behavior','Computational modelling','TrialValues','Choice','HR'); % use later for model-derived variables CPP/RU

% output paths
padi.behav = fullfile(padi.data);
padi.meanEPI = fullfile(padi.anat,'meanEPI');
padi.stats = fullfile(padi.data,'stats','fMRI');
